...
public class BlockManager implements BlockStatsMXBean {
  ...
  private long startRecovery = 0;
  private long endRecovery = 0;
  private int repairedBlocks = 0;
  
  public void mapBlkId2FileName(long blkid, String filename) {
    this.blkId2FileName.put(new Long(blkid), filename);
  }
  
  public String getFileNameWithBlkId(long blkid) {
    return this.blkId2FileName.get(new Long(blkid));
  }
  ...
  public DatanodeStorageInfo[] chooseTarget4NewBlockFromOEC(final String src,
      ...
      final BlockType blockType,
      final ErasureCodingPolicy ecPolicy,
      final EnumSet<AddBlockFlag> flags, Block b) throws IOException {
    List<DatanodeDescriptor> favoredDatanodeDescriptors =
        getDatanodeDescriptors(favoredNodes);
    final BlockStoragePolicy storagePolicy =
        storagePolicySuite.getPolicy(storagePolicyID);
    ...
    final DatanodeStorageInfo[] targets = blockplacement.chooseTarget(src,
        numOfReplicas, client, excludedNodes, blocksize,
        favoredDatanodeDescriptors, storagePolicy, flags, b);
      ...
      if (count.intValue() == zero.intValue()) {
        ...
        if (useOEC) {
          for (int i=0; i<list.size(); i++) {
            ...
            long blkid = item.longValue();
            neededReconstruction.deleteFromOECRepairList(blkid);
          }
        }
        ...
        this.corruptObjStat.put(filename, count);
      }
      if (useOEC) neededReconstruction.printOECList();
    }

    LOG.info("BlockManager.addBlock 3898.corruptObjStat.size = " + corruptObjStat.size());
    LOG.info("BlockManager.addBlock 3898.startRecovery = " + startRecovery);
    if ((this.corruptObjStat.size() == 0) && (this.startRecovery != 0)) {
      this.health = true;
 ...
 public String getCorruptReason(Block block, DatanodeDescriptor node) {
  ...
  public boolean addCorruptObj(BlockInfo bi) {
    boolean useOEC = conf.getBoolean("link.oec", false);
    Integer one = new Integer(1);

    String blkName = bi.getBlockName();
    long blkId = bi.getBlockId();
    String objname = blkId2FileName.get(blkId);

    if (objname.contains("tmp") ||
        objname.contains("io_control") ||
        objname.contains("part") ||
        objname.contains("_SUCCESS")) return false;

    if (this.corruptObjStat.containsKey(objname)) {
//      Integer count = this.corruptObjStat.get(objname);
//      List<Long> list = this.corruptObj2Blks.get(objname);
//      list.add(blkId); 
//      this.corruptObj2Blks.put(objname, list);
//      count = count+one;
//      this.corruptObjStat.put(objname, count);
    } else {
      System.out.println("BlockManager::addCorruptObj.add " + blkName + " of file " + objname + " into corruptObj");
      this.corruptObjStat.put(objname, new Integer(1));
    ...
    System.out.println("BlockManager::addCorruptObj.add " + blkName + " of file " + objname + " into corruptObj");
    if (health) {
      // this is the time to start repairing
      health = false;
      startRecovery = System.currentTimeMillis();
      repairedBlocks = 0;
      System.out.println("BlockManager::addCorruptObj.start recovery");
    }
    return true; 
  }
    ...
    public void run() {
      while (namesystem.isRunning()) {
        try {
          ...
          boolean enableRecovery = conf.getBoolean("enable.recovery", true);
          if (isPopulatingReplQueues()) {
            if (enableRecovery) {
              computeDatanodeWork();
  ...
  public void repairByOEC() {
    ...
    int max = 10;
    Integer one = new Integer(1);
//    System.out.println("BlockManager::repairByOEC.corruptsize = " + neededReconstruction.oecCorruptSize() + ", repairsize = " + neededReconstruction.oecRepairSize());
    while ( (neededReconstruction.oecCorruptSize() > 0) ) {
      neededReconstruction.printOECList();
      BlockInfo curBI = neededReconstruction.getFirstOECCorruptList();
      System.out.println("BlockManager::repairByOEC.check corruptblk " + curBI.getBlockName());
      if (addCorruptObj(curBI)) {
        String blkname = curBI.getBlockName();
        long blkId = curBI.getBlockId();
        String objname = blkId2FileName.get(blkId);
        System.out.println("BlockManager::repairByOEC.send repair req for 4543.blkId: " + blkId + ", objname: " + objname);
        // send repair request to OpenEC
        CoorCommand coorCmd = new CoorCommand();
        coorCmd.buildType6(6, localAddr, objname);
        coorCmd.sendTo(oecCoorAddr);
      } else {
        System.out.println("BlockManager::repairByOEC.ignore current lost");
      }
      neededReconstruction.addToOECRepairList(curBI);
      neededReconstruction.deleteFromOECCorruptList(curBI);
    }
  }
