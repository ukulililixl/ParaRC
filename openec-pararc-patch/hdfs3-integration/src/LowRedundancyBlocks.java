...
class LowRedundancyBlocks implements Iterable<BlockInfo> {
  ...
  int oecCorruptSize() {
    return this.oecCorruptList.size();
  }

  int oecRepairSize() {
    return this.oecRepairList.size();
  }

  BlockInfo getFirstOECCorruptList() {
    return this.oecCorruptList.get(0);
  }
 
  void deleteFromOECCorruptList(BlockInfo bi) {
    this.oecCorruptList.remove(bi);
  }
   
  void addToOECRepairList(BlockInfo bi) {
    this.oecRepairList.add(bi);
  }

  void deleteFromOECRepairList(long blkid) {
    Iterator<BlockInfo> iterator = this.oecRepairList.iterator();
    while (iterator.hasNext()) {
      BlockInfo bi = iterator.next();
      if (bi.getBlockId() == blkid) {
        iterator.remove();
        break;
      }
    }
  ...
  void printOECList() {
    String corrupt = "oecCorruptList: ";
    for (int i=0; i<this.oecCorruptList.size(); i++) {
      BlockInfo bi = this.oecCorruptList.get(i);
      corrupt += bi.getBlockId() + " ";
    }
    String repair = "oecRepairList: ";
    for (int i=0; i<this.oecRepairList.size(); i++) {
      BlockInfo bi = this.oecRepairList.get(i);
      repair += bi.getBlockId() + " ";
    }
  ...
  synchronized List<List<BlockInfo>> chooseLowRedundancyBlocks(
    ...
    for (; count < blocksToProcess && priority < LEVEL; priority++) {
      ...
      for(; count < blocksToProcess && i.hasNext(); count++) {
        ...
        if (useOEC) {
          if (this.oecCorruptList.contains(block) || this.oecRepairList.contains(block)) continue;
          else this.oecCorruptList.add(block);
        }
      }
