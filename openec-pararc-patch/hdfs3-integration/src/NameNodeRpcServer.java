...
//    System.out.println("XL::NameNodeRpcServer.addBlock 864") ;
    ...
    /* XL: maybe we can hack like this
       if filename has string "OECOBJ" 
         ...
         this is not a OECOBJ file, get location from HDFS itself
       endif
     */
    long startnano = System.nanoTime();
    long startmilli = System.currentTimeMillis();
    LocatedBlock locatedBlock;
    boolean linkOEC = this.conf.getBoolean("link.oec", false);
    if (src.contains("/benchmarks/IOTest/io_control/") ||
        src.contains("oecobj") ||
        src.contains("oecstripe") ||
        src.contains("placetest") ||
        src.contains("offlinepool") ||
        src.contains("workGenControl")) {
      // This is the control file for IOTest, have a influence on where mapper launches
      locatedBlock = namesystem.getAdditionalBlockFromOEC(src, fileId,
            clientName, previous, excludedNodes, favoredNodes, addBlockFlags);
    } else {
      // We do not modify where it stores
      locatedBlock = namesystem.getAdditionalBlock(src, fileId,
            clientName, previous, excludedNodes, favoredNodes, addBlockFlags);
    }
...
//        clientName, previous, excludedNodes, favoredNodes, addBlockFlags);
    ...
    long blkId = locatedBlock.getBlock().getBlockId();

    if (locatedBlock.isStriped()) {
      ExtendedBlock currentBlockGroup = locatedBlock.getBlock();
      // try to simulate blkids?
      byte[] indices = ((LocatedStripedBlock)locatedBlock).getBlockIndices();
      for (int i=0; i<indices.length; i++) {
        int idx = indices[i];
        long curblkid = blkId + idx;
        namesystem.mapBlkId2FileName(curblkid, src);
      }
    } else {
      namesystem.mapBlkId2FileName(blkId, src);
    }
    System.out.println("NameNodeRpcServer.addBlock.getLocatedBlock.time = " + (endnano-startnano) + " nanosecond = " + (endmilli-startmilli) + " millisecond");

    // TODO: add a map from LocatedBlock?->filename;
    namesystem.mapBlkId2FileName(blkId, src);
    return locatedBlock;
  }
