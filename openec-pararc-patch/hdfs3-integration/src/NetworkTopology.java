...
public class NetworkTopology {
  ...
  public Node[] getLocFromOEC(String name, String type, String policy, int numOfReplicas, Node writer) {
    Node[] toret = new Node[numOfReplicas];
    if (numOfReplicas == 1)  toret[0] = getLocFromOEC(name, type, policy);
    else {
      toret[0] = writer;
      for (int i=1; i<numOfReplicas; i++) {
        toret[i] = getLocFromOEC(name, type, policy);
      }
    }
    return toret;
  }
  ...
  public Node getLocFromOEC(String name, String type, String policy) {
    if (policy.equals("random")) {
      ...
      if (type.equals("oecobj")) {
        ...
        for (int i=0; i<this.oecIpList.size(); i++) {
          if (curStripeIdx.contains(i)) continue;
          candidate.add(i);
        }
      ...
      loadMap.put(new Integer(randomidx), loadNum + new Integer(1));
      return this.oecIpList.get(randomidx);
    } else {
      Integer miniload = this.loadMap.get(new Integer(0));
      int miniidx = 0;
      if (miniload == null) {
        miniload = new Integer(0);
        this.loadMap.put(new Integer(miniidx), miniload);
      }
      
      for (int i=1; i<this.oecIpList.size(); i++) {
        if (type.equals("oecobj")) {
          List<Integer> avoid = file2IpMap.get(name);
          if (avoid == null) {
            avoid = new LinkedList<Integer>();
            file2IpMap.put(name, avoid);
          }
          // first check whether current idx is in avoid
          if (avoid.contains(i)) continue;
        }
        // compare with mini
        Integer curload = loadMap.get(i);
        if (curload == null) {
        ...
        if (curload < miniload) {
          ...
          miniidx = i;
//          loadMap.put(new Integer(i), miniload + new Integer(1));
          break;
        }
      }
      loadMap.put(new Integer(miniidx), miniload + new Integer(1));

      // update file2IPMap
      List<Integer> list = file2IpMap.get(name);
  ...
  public void add(Node node) {
    ...
    int idx = loc.indexOf(":");
    String ip;
    if (idx != -1) ip = loc.substring(0, idx);
    else ip = loc;
    System.out.println("NetworkTopology.add.loc = " + loc + ", ip = " + ip);
    if (!this.ipMap.containsKey(ip)) {
      this.ipMap.put(ip, node);
    }
