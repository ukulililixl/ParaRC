...
//        case 21: registerLayerFile(coorCmd); break;
        default: break;
      }
      delete coorCmd;
    }
...
//}
//
//void Coordinator::registerLayerFile(CoorCommand* coorCmd) {
//  unsigned int clientIp = coorCmd->getClientip();
...
//  string layer = coorCmd->getLayer();
//  cout << "Coordinator::registerLayerFile.filename = " << filename << ", ecid = " << ecid << ", mode = " << mode << ", filesizeMB = " << filesizeMB << ", layer = " << layer << endl;
//
//  if (mode == 0) registerLayerOnlineEC(clientIp, filename, ecid, filesizeMB, layer);
//  else if (mode == 1) registerLayerOfflineEC(clientIp, filename, ecid, filesizeMB, layer);
//}
//
//void Coordinator::registerLayerOnlineEC(unsigned int clientIp, string filename, string ecid, int filesizeMB, string layer) {
//    cout << "Coordinator::registerLayerOnlineEC" << endl;
//  // 0. make sure that there is no existing ssentry
//  assert (!_stripeStore->existEntry(filename));
//  // 1. get ec instance
//  ECPolicy* ecpolicy = _conf->_ecPolicyMap[ecid];
...
//  vector<string> objnames;
//  for (int i=0; i<ecn; i++) {
//    // TODO: filename_oecobj_idx_layer
//    //string obj = filename+"_oecobj_"+to_string(i);
//    string obj = filename+"_oecobj_"+to_string(i)+"_"+layer;
//    objnames.push_back(obj);
//    vector<int> colocWith;
//    if (idx2group.find(i) != idx2group.end()) colocWith = idx2group[i];
//    vector<unsigned int> candidates = getCandidates(ips, placed, colocWith);
...
//  }
//  // 5. update ssentry and stripestore
//  // TODO: create ssentry with layer information
//  //SSEntry* ssentry = new SSEntry(filename, 0, filesizeMB, ecid, objnames, ips);
//  SSEntry* ssentry = new SSEntry(filename, 0, filesizeMB, ecid, objnames, ips, layer);
//  _stripeStore->insertEntryWithLayer(ssentry, layer);
//  ssentry->dump();
//  // 6. parse ECDAG and create commands for online encoding
//  ECDAG* ecdag = ec->Encode();
//  vector<int> toposeq = ecdag->toposort();
...
//}
//
//void Coordinator::registerLayerOfflineEC(unsigned int clientIp, string filename, string ecpoolid, int filesizeMB, string layer) {
//  cout << "Coordinator::registerLayerOfflineEC" << endl;
//  struct timeval time1, time2, time3, time4;
//  // 0. make sure that there is no existing ssentry
//  assert (!_stripeStore->existEntry(filename));
//
...
//
//  for (int objidx=0; objidx<objnum; objidx++) {
//    string objname = filename+"_oecobj_"+to_string(objidx) + "_" + layer;
//    fileobjnames.push_back(objname);
//
//    // 4.1 get a stripe for obj
//    //   we get a stripename from ecpool, however ecpool does not add objname into it at this time
...
//    for (int i=0; i<stripeobjlist.size(); i++) {
//      string curobjname = stripeobjlist[i];
//      cout << "Coordinator::registerLayerOfflineEC:curobjname = " << curobjname << endl;
//      // given curobjname, find ssentry of the original file for this curobjname
//      // if this curobjname is from previous stored file, there must be an SSEntry in stripestore
//      // else ssentry is NULL
//      SSEntry* ssentry = _stripeStore->getEntryFromObj(curobjname);
...
//      bool find = false;
//      if (ssentry != NULL) {
//        cout << "Coordinator::registerLayerOfflineEC:ssentry != NULL" << endl;
//        //curip = ssentry->getLocOfObj(curobjname);
//        curip = ssentry->getLocOfObjWithTier(curobjname, layer);
//        find = true;
//      } else {
//        cout << "Coordinator::registerLayerOfflineEC:ssentry == NULL" << endl;
//        // curobjname is in current file
...
//
//  // 2. update ssentry
//  SSEntry* ssentry = new SSEntry(filename, 1, filesizeMB, ecpoolid, fileobjnames, fileobjlocs, layer);
//  _stripeStore->insertEntryWithLayer(ssentry, layer);
//  ssentry->dump();
//
//  ecpool->unlock();
...
//
//  memset(toret, 0, numOfReplicas*sizeof(unsigned int));
//
//  string netobjname = objname;
//  bool proceed = true;
//  if (_conf->_fsType == "CRAIL") {
//      string oriobjname = _stripeStore->hashHit(objname);
//      if (oriobjname.size() == 0)
//          proceed = false;
//      else
//          objname = oriobjname;
//  }
//  cout << "getFileLocation: objname = " << objname << endl;
//
//  if (proceed) {
//	  // 0. figure out the objtype
//	  if (objname.find("oecobj") != string::npos) {
//		  vector<string> items = RedisUtil::splitStr(objname, "_");
//
//		  string filename = items[0];
//		  string idxstr = items[2];
//		  int idx = atoi(idxstr.c_str());
//		  cout << "filename: " << filename << ", idx: " << idx << endl;
//
//		  bool tier = false;
//		  string layer;
//		  if (items.size() > 3) {
...
//	  cout << RedisUtil::ip2Str(toret[i]) << " " << "int = " << (int)toret[i] << " ";
//  }
//  cout << endl;
//  // last. return ip
//  redisReply* rReply;
...
//    if (!ssentry->useTier())
//        objlist = ssentry->getObjlist();
//    else {
//        string tier = ssentry->getTier();
//        objlist = ssentry->getLayerObjList(tier);
//    }
//  
...
//    else {
//        usetier = true;
//        loc = ssentry->getLocOfObjWithTier(objname, ssentry->getTier());
//        layer = ssentry->getTier();
//    }
...
//    SSEntry* ssentry;
//    if (!usetier)
//        ssentry = new SSEntry(objname, 1, basesizeMB, ecpoolid, {objname}, {loc});
//    else
//        ssentry = new SSEntry(objname, 1, basesizeMB, ecpoolid, {objname}, {loc}, layer);
//
//    if (_conf->_fsType == "CRAIL") {
//        _stripeStore->insertEntryWithLayer(ssentry, layer);
//    } else {
...
//
//  // generate ip for virtual leaf vertices
...
//    if (sidx >= n)
//        virleaves[idx] = 1;
//    if (sid2ip.find(sidx) != sid2ip.end())
//        continue;
//    if (sidx >= n) {
//        vector<unsigned int> candidates = _conf->_agentsIPs;
//        unsigned int ip = chooseFromCandidates(candidates, _conf->_encode_policy, "encode");
//        sid2ip[sidx] = ip;
//    }
//  }
//
...
//    if (virleaves.size() == 0)
//        candidates = node->candidateIps(sid2ip, cid2ip, _conf->_agentsIPs, n, k, w, locality);
//    else
//        candidates = node->candidateIpsWithVirtualLeaves(sid2ip, cid2ip, _conf->_agentsIPs, virleaves, n, k, w, locality);
//
...
////  }
//
//  // check whether persist item is used
//  vector<int> curheaders = ecdag->getHeaders();
//
//  for (int ii=k; ii<n; ii++) {
//      for (int jj=0; jj<w; jj++) {
//          int cid = ii * w + jj;
//          if (find(curheaders.begin(), curheaders.end(), cid) == curheaders.end()) {
//              ecdag->getNode(cid)->incRefNumFor(cid);
//          }
//      }
//  }
//
...
//  if (virleaves.size() == 0)
//      agCmds = ecdag->parseForOEC(cid2ip, stripename, n, k, w, pktnum, objlist);
//  else
//      agCmds = ecdag->parseForOECWithVirtualLeaves(cid2ip, virleaves, stripename, n, k, w, pktnum, objlist);
//
//  // 7. add persist cmd
...
//  if (!usetier)
//      persistCmds = ecdag->persistWithInfo(cid2ip, stripename, n, k, w, pktnum, objlist, plist);
//  else
//      persistCmds = ecdag->persistLayerWithInfo(cid2ip, stripename, n, k, w, pktnum, objlist, plist, layer);
//
...
//  redisAppendCommand(distCtx, "MULTI");
//
//  vector<int> sortres = ecdag->toposort();
//  int debug=0;
//  for (int i=0; i<sortres.size(); i++) {
//      int cid = sortres[i];
//      if (agCmds.find(cid) == agCmds.end())
//          continue;
//
//      //if (debug > 52)
//      //    break;
//
//      auto agcmd = agCmds[cid];
//      unsigned int ip = agcmd->getSendIp();
//      ip = htonl(ip);
//
...
//  //TODO: attach layer information?
//  bool usetier = ssentry->useTier();
//  memcpy(filemeta + metaoff, (char*)&usetier, 1); metaoff += 1;
//  string layer;
//  if (usetier) {
//	  layer = ssentry->getFastLayer();
//	  int layersize = layer.size();
//	  int tmplayersize = htonl(layersize);
//	  memcpy(filemeta + metaoff, (char*)&tmplayersize, 4); metaoff += 4;
//	  memcpy(filemeta + metaoff, layer.c_str(), layer.size()); metaoff += layer.size();
//  }
//
//  if (redundancy == 0) {
//    // for onlineec
...
//	  vector<string> objlist;
//	  if (usetier)
//		  objlist = ssentry->getLayerObjList(layer);
//	  else
//		  objlist = ssentry->getObjlist();
...
//    vector<unsigned int> candidates;
//    if (virleaves.size() == 0)
//        candidates = node->candidateIps(sid2ip, cid2ip, _conf->_agentsIPs, ecn, eck, ecw, locality, lostidx);
//    else
//        candidates = node->candidateIpsWithVirtualLeaves(sid2ip, cid2ip, _conf->_agentsIPs, virleaves, ecn, eck, ecw, locality, lostidx);
//    
//    // choose from candidates
//    unsigned int curip = chooseFromCandidates(candidates, _conf->_repair_policy, "repair");
//    cid2ip.insert(make_pair(cidx, curip));
...
//  unordered_map<int, AGCommand*> agCmds;
//  if (virleaves.size() == 0)
//      agCmds = ecdag->parseForOEC(cid2ip, stripename, ecn, eck, ecw, pktnum, objlist);
//  else
//      agCmds = ecdag->parseForOECWithVirtualLeaves(cid2ip, virleaves, stripename, ecn, eck, ecw, pktnum, objlist);
//  
//  // 7. add persist cmd
...
//  //int objsizeMB = filesizeMB / eck;
//  
//  int objsizeMB = ecpool->getBasesize();
//  int pktnum = objsizeMB * 1048576/_conf->_pktSize;
//
//  // optimize 
//  ecdag->optimize2(opt, cid2ip, _conf->_ip2Rack, ecn, eck, ecw, sid2ip, _conf->_agentsIPs, locality);
