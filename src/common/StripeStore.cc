...
void StripeStore::lockLoadVector() {
  _lockLoadVector.lock();
}

void StripeStore::unlockLoadVector() {
  _lockLoadVector.unlock();
}

void StripeStore::getLoadValueFor(LoadVector* lv) {
  vector<unsigned int> ips = lv->getNodeList();
  unordered_map<unsigned int, int> innum;
  unordered_map<unsigned int, int> outnum;
  unordered_map<unsigned int, int> calnum;

  for (auto ip: ips) {
    innum.insert(make_pair(ip, _loadVector->getInValueFor(ip)));
    outnum.insert(make_pair(ip, _loadVector->getOutValueFor(ip)));
    calnum.insert(make_pair(ip, _loadVector->getCalValueFor(ip)));
  }

  lv->updateLoadValue(innum, outnum, calnum);
}

void StripeStore::updateLoadVector(LoadVector* lv) {
   _loadVector->updateLoadValue(lv->getInNum(),
                                lv->getOutNum(),
                                lv->getCalNum());
}
...
//}
//
//void StripeStore::insertEntryWithLayer(SSEntry* entry, string layer) {
//  unordered_map<string, SSEntry*>::iterator it = _ssEntryMap.find(entry->getFilename());
//  if (it == _ssEntryMap.end()) {
//    // the entry does not exist, insert this entry
//    _lockSSEntryMap.lock();
//    _ssEntryMap.insert(make_pair(entry->getFilename(), entry));
//    cout << "StripeStore::insertEntryWithLayer.insert entry for " << entry->getFilename() << endl;
//    _lockSSEntryMap.unlock();
//
//    for (auto obj: entry->getLayerObjList(layer)) {
//      _lockObjEntryMap.lock();
//      _objEntryMap.insert(make_pair(obj, entry));
//      cout << "StripeStore::insertEntryWithLayer.insert entry for " << obj << endl;
//      _lockObjEntryMap.unlock();
//    }
//  }
//
//  if (_conf->_fsType == "CRAIL") {
//    // add all objhash to _crail_hashes
//      for (auto obj: entry->getLayerObjList(layer)) {
//          string hashvalue = RedisUtil::fileHash(obj);
//          _lockCrailHashes.lock();
//          _crail_hashes.insert(make_pair(hashvalue, obj));
//          _lockCrailHashes.unlock();
//          cout << "record obj: " << obj << ", hash: " << hashvalue << endl;
//      }
//  }
//
//  if (_entry2Layer.find(entry->getFilename()) == _entry2Layer.end()) {
//      _lockEntry2Layer.lock();
//      _entry2Layer.insert(make_pair(entry->getFilename(), layer));
//      _lockEntry2Layer.unlock();
//
//      _lockLayer2Entries.lock();
//      if (_layer2Entries.find(layer) == _layer2Entries.end()) {
//          vector<string> curlist;
//          curlist.push_back(entry->getFilename());
//          _layer2Entries.insert(make_pair(layer, curlist));
//      } else {
//          _layer2Entries[layer].push_back(entry->getFilename());
//      }
//      _lockLayer2Entries.unlock();
//  }
//}
//
//string StripeStore::getLayer(SSEntry* entry) {
//    string entryname = entry->getFilename();
//    string toret;
//    if (_entry2Layer.find(entryname) != _entry2Layer.end())
//        toret = _entry2Layer[entryname];
//    return toret;
//}
//
...
//}
//
//void StripeStore::addHash(string objname) {
//    string hashvalue = RedisUtil::fileHash(objname);
//    _lockCrailHashes.lock();
//    _crail_hashes.insert(make_pair(hashvalue, objname));
//    _lockCrailHashes.unlock();
//}
//
//string StripeStore::hashHit(string hashstring) {
//    string toret;
//    cout << "check hash: " << hashstring << endl;
//    if (_crail_hashes.find(hashstring) != _crail_hashes.end())
//        toret = _crail_hashes[hashstring];
//    return toret;
//}
