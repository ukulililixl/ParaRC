...
//    else if (_childNodes.size() == 1) {
//        ECNode* curnode = _childNodes[0];
//        if (curnode->isOptNode())
//            computebool = false;
//        else
//            computebool = true;
//    }
...
//        int cidx = _childNodes[0]->getNodeId();                                                                                                                                                                                                                                   
//        assert (cid2ip.find(cidx) != cid2ip.end());                                                                                                                                                                                                                               
//        toret.push_back(cid2ip[cidx]);                                                                                                                                                                                                                                            
//    } else {                                                                                                                                                                                                                                                                      
//        toret = allIps;                                                                                                                                                                                                                                                         
...
//}
//
//vector<unsigned int> ECNode::candidateIpsWithVirtualLeaves(unordered_map<int, unsigned int> sid2ip,
//                                          unordered_map<int, unsigned int> cid2ip,
//                                          vector<unsigned int> allIps,
...
//
//  for (int i=0; i<_childNodes.size(); i++) {
//      int childCidx = _childNodes[i]->getNodeId();
//      assert (cid2ip.find(childCidx) != cid2ip.end());
//
//      if (virleaves.find(childCidx) == virleaves.end())
//          continue;
//
//      // child is a virtual idx, and is a leaf vertex
//      toret.push_back(cid2ip[childCidx]);
//      return toret;
//  }
//
//  // 2. if locality is enabled prepare candidate from children
...
//}
//
//void ECNode::parseForOECWithVirtualLeaves(unsigned int ip,
//        unordered_map<int, int> virleaves, int w) {
//  _ip = ip;
//  bool cache = true;
//
//  // -1. check for virleaves
//  bool shorten = false;
//  if (virleaves.find(_nodeId) != virleaves.end()) {
//      // this is a virtual leave task
//      ECTask* shorttask = new ECTask();
//      shorttask->setType(-1);
//      shorttask->addIdx(_nodeId);
//      _oecTasks.insert(make_pair(-1, shorttask));
//      shorten = true;
//  }
//
//  // 0. check for Load
...
//  bool cache = false;
//
//  if (_oecTasks.find(-1) != _oecTasks.end()) shorten = true;
//  if (_oecTasks.find(0) != _oecTasks.end()) load = true;
//  if (_oecTasks.find(1) != _oecTasks.end()) fetch = true;
//  if (_oecTasks.find(2) != _oecTasks.end()) compute = true;
//  if (_oecTasks.find(3) != _oecTasks.end()) cache = true;
////  cout << "ECNode::parseAGCommand.load: " << load << ", fetch: " << fetch << ", compute: " << compute << ", cache:" << cache << endl;
// 
//  if (shorten & !load & !fetch & !compute & cache) {
//    vector<int> indices = _oecTasks[-1]->getIndices();
//    AGCommand* agCmd = new AGCommand();
//    agCmd->buildType23(23, _ip, stripename, w, num, indices, _oecTasks[3]->getRefMap());
//    return agCmd;
//  }
//
//  if (!shorten & load & !fetch & !compute & cache) {
