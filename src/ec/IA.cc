...
//    cout << "to recover block " << rBlkIdx << endl;
    ...
    // XL update
    vector<int> new_rgdata;
    vector<int> new_rgcoef;
    for (int i=0; i<rgcoef.size(); i++) {
      if (rgcoef[i] != 0) {
        new_rgcoef.push_back(rgcoef[i]);
        new_rgdata.push_back(rgdata[i]);
      }
    }
    ecdag->Join(rUnitIdx, new_rgdata, new_rgcoef);
  } 
//  ecdag->BindX(to);
  return ecdag;
}
